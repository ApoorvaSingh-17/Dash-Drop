...
const MapAutoFocus = ({ position }) => {
  ...
  React.useEffect(() => {
    if (position) {
      map.setView(position, 13); // Pan + zoom to the new position
    }
  }, [position, map]);
...
const UpdateLocationFormTwo = () => {
  ...
  // Auto-geocode on city and state change
  useEffect(() => {
    const fetchCoordinates = async () => {
      const { address, city, state, zip } = form;
      if (address && city && state && zip) {
        const locationString = `${address}, ${city}, ${state}, ${zip}`;
        try {
          const response = await axios.get(`https://api.opencagedata.com/geocode/v1/json`, {
            params: {
              key: '8762e4c3711b4ddcac9b7e9bbb72db5d',
              q: locationString,
              limit: 1,
            },
          });
      ...
      <div className='imageHomeUpdate centre'>
        ...
        <div className="mt-4">
          ...
          >
            ...
            {selectedLocation && <MapAutoFocus position={[selectedLocation.lat, selectedLocation.lng]} />}
...
export default UpdateLocationFormTwo;
